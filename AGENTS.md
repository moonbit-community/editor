...
- Product code lives at the repository root in MoonBit packages: `base/common`, `decorations`, `workspace`, `language`, `platform/log`, `syntax`, `syntax/lang_*`, `viewer`, `viewer/common`, `viewer/core`, `viewer/model`, `viewer/view_line_renderer`, `viewer/view_model`, `viewer/view_layout`, `widgets/file_tree`, `workbench`, `web`, `remote_protocol`, `server`, `server_host_native`, `view`, and `examples/embedded_viewer`.
...
- Packages that only run on one host target may declare that host's FFI (for example, `viewer` may declare JavaScript FFI and use `rabbita/dom`; `server_host_native` may declare native FFI). Packages shared across targets must not declare FFI.
