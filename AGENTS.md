...
- Public product code lives at the repository root in MoonBit packages:
  `base/common`, `decorations`, `language`, `platform/log`, `syntax`,
  `syntax/lang_*`, `viewer`, `viewer/common`, `viewer/core`, `viewer/cursor`,
  `viewer/model`, `viewer/view_line_renderer`, `viewer/view_model`,
  `viewer/view_layout`, and `view`.
