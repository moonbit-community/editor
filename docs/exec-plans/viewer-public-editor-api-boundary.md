...
- `ViewZone` and `ViewZoneChangeAccessor` from
...
selection/cursor/scroll events, view-zone API, contribution lookup, and service
contracts. Every sibling cluster—editing, diff editor, minimap, accessibility,
commands not present locally—is explicitly `N-A` or `DEFERRED`, not omitted.
...
- readonly editor option/service capability records that are real host seams;
- other DOM-free public enums/records proven by the disposition inventory.
...
- content/overlay widget contracts;
...
`ViewerServices` fields become private and use public capability contracts at
the host boundary. Concrete contribution implementation types do not appear
in its generated interface. Capability records are justified here because
this is a real dependency-injection/host seam; they must contain only methods
proven by call-site inventory.
...
| `ViewZone` alias from `view_parts` | public descriptor in `viewer/browser`; private rendered state in `browser/view` |
...
## Milestone C: Separate Public View-Zone Contract from Runtime State
...
2. Replace internal option-type imports in its public signature with canonical
   `editor_api` types.
3. Inventory every concrete `ViewerServices` operation consumed by root and
   contributions.
4. Define minimal public host capability records in `editor_api`; adapt
   concrete language/marker/feature services at the root/workbench boundary.
5. Make `ViewerServices` fields private and remove concrete contrib/browser
   implementation types from `viewer/pkg.generated.mbti`.
6. Update workbench code to import feature implementations directly where it
   owns them, while the external embedding example uses only the public facade
   and public contract packages.
...
6. update examples to demonstrate the intended public construction and service
   boundary.
...
- content/overlay widgets and view zones through browser contracts;
