...
- Declare JavaScript FFI for browser document state, filesystem provider hooks,
  LSP transport hooks, remote protocol transport hooks, rendering, callbacks,
  disposables, and observability.
- Convert host responses into MoonBit `workspace` and `language` contracts.
...
- May depend on `workspace`, `language`, and JS async support.
