...
`viewer/ui/scrollbar.ScrollableElementDom`. It exposes the wrapper, shared
content node, and scrollable object needed by `View` and the root input bridge.
Scrollbar DOM, slider arithmetic, reveal/fade, wheel normalization, and drag
geometry remain owned by `viewer/ui/scrollbar`; model scroll state remains in
...
provenance remain byte-for-byte stable. The remaining
`viewer/browser/view_parts/*` directories are CSS asset locations, not
MoonBit packages. Root editor CSS and the codicon font remain under this
directory.
